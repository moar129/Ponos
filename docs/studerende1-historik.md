...
**Rolle-begrænsede opgaverum (2026-09-25, cross-domain, godkendt af bruger):** Opgaver er Studerende 3's domæne. Brugeren bad eksplicit om at bygge det selv. Et rum kan nu begrænses til en eller flere roller via den nye join-tabel `task_room_roles`, som erstatter den ubrugte `task_rooms.required_role_id` (droppet). Uden en af rollerne er rummet OG dets opgaver skjult i RLS (`can_access_task_room`). Undtagelser: admin, det nye privilegie `view_all_task_rooms` og tilmeldte (`is_task_assignee`). Rum uden roller er åbne som før. Slettes en rolle, cascader den ud, og står rummet uden roller, bliver det åbent (accepteret). Skrivning sker kun via RPC'erne `create_task_room`/`update_task_room`. Frontend: `RoomRolePicker.tsx` (checkbox-liste) i opret-rum (TaskPage + MyTasksPage) og `EditRoomModal`, samt 🔒 + tooltip i `RoomBar`. SQL kørt 2026-09-25 og foldet ind i `dbSchema.sql` (§9.5, §15.22, §16.7, §16.7b), migrationsfilen slettet. Testet i browseren og godkendt af bruger 2026-09-25.

**Hurtig rolle-oprettelse fra rum-modalen (2026-09-25):** "+ Ny rolle" i `RoomRolePicker` (gated på `create_roles`) åbner `QuickCreateRoleModal.tsx` (`src/components/dashboard/roles/`). Den har skabeloner: Deltager = `read_tasks` + `read_news` (Medlems basis), Leder = Deltager + opret/rediger/tildel/godkend/afvis opgaver, og Kopiér fra rolle (kræver `read_roles`). Privilegierne kan tilpasses grupperet efter `PRIVILEGE_DOMAINS`, og den nye rolle krydses automatisk af på rummet. Navnet forudfyldes med rumnavnet (+ " Leder"). Ny RPC `create_role_with_privileges` kræver `create_roles`, fordi privileges-insert via RLS ellers kræver `update_roles`. Eskalerings-guard: man kan kun give privilegier, man selv har (admin: alle), og aldrig `admin`. SQL kørt 2026-09-25, foldet ind i `dbSchema.sql` §15.23, migrationsfilen slettet. Testet i browseren og godkendt af bruger 2026-09-25.
