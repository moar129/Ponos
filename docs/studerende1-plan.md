...
**US-68 + US-69 – Nulstil og skift adgangskode**

Begge stories er oprettet i `userStories.md` 2026-09-11, og den fulde implementeringsspec ligger i afsnittet "US-68 + US-69 – Adgangskode (spec, ikke bygget endnu)" nederst. **Der er endnu ikke skrevet kode, og SQL'en er ikke kørt** - dokumentationen er lavet først efter aftale med bruger.

To stories, fordi det er to forskellige situationer: US-68 er den udloggede vej ind igen (glemt adgangskode), US-69 er den indloggede vej til at skifte den. US-68 er efter bruger-beslutning en **prototype-løsning uden mail** - se forbeholdet i specen og i US-68's acceptkriterier, det skal ikke overses.

**Derefter: US-62 + US-63 – Granulære skriverettigheder i Datalayer/Opgaver (Fase 2)**
...
| US-02 | Login | Critical | Delvist | Login/redirect virker; banner viser nu "ingen organisation" + link til anmodning; `/`-ruten er stadig ikke beskyttet. En glemt adgangskode kan man ikke selv nulstille - det er udskilt som US-68, og "skift adgangskode" som US-69 |
...
| US-68 | Nulstil adgangskode | High | Mangler | Ny story, tilføjet ad-hoc efter bruger-forespørgsel 2026-09-11. Opdaget under arbejdet med `/hjaelp`: der findes intet `resetPasswordForEmail`/`updateUser` i `src/`, så FAQ-posten "Jeg har glemt min adgangskode" måtte skrives som "skriv til os". **Specificeret, ikke bygget** - fuld spec i afsnittet "US-68 + US-69 – Adgangskode" nederst. **Prototype-forbehold (bruger-beslutning):** hele flowet foregår på hjemmesiden uden mail, fordi projektet ikke deployes. Det betyder at identitetstjekket kun er email + fornavn + efternavn, og at RPC'en er `grant`et til `anon` - enhver, der kender en brugers navn og email, kan overtage kontoen. Bevidst valgt, men **skal erstattes af et mailbaseret flow, før noget sættes i drift**. Flowet er specificeret, så kun RPC-kaldet skal skiftes den dag. |
| US-69 | Skift adgangskode | Medium | Mangler | Ny story, tilføjet ad-hoc efter bruger-forespørgsel 2026-09-11, sammen med US-68. Den indloggede halvdel: `/bruger` kan i dag rette navn/beskrivelse/billede, men ikke adgangskoden. Ingen SQL - ren Supabase Auth. Har **ikke** US-68's prototype-forbehold: brugeren er logget ind og bekræfter sin nuværende adgangskode, så identiteten er faktisk bevist. **Specificeret, ikke bygget** - se samme spec-afsnit. |
...
**Fundet undervejs (ikke rettet):** der findes **ingen nulstil-adgangskode-funktion** i appen — intet `resetPasswordForEmail`/`updateUser` nogen steder i `src/`. FAQ-posten henviser derfor til mail. Bruger har besluttet, at nulstilling bliver en **egen user story senere** — det blev US-68 (og US-69 for den indloggede variant), se spec-afsnittet nedenfor; FAQ-posten skal rettes, når de bygges.

Ingen SQL, ingen RLS, ingen nye dependencies eller endpoints.

## US-68 + US-69 – Adgangskode (spec, ikke bygget endnu)

Skrevet 2026-09-11 efter aftale med bruger: **dokumentationen først, koden bagefter.** Intet af nedenstående er udført — ingen filer i `src/` er rørt, og SQL'en er ikke kørt. Afsnittet er skrevet, så en frisk session kan udføre det uden at genudlede noget.
