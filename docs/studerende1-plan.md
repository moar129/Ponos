...
**2. US-70 – Se afsluttede opgaver. ← NÆSTE OPGAVE.** Kun storyen er skrevet (14/09) - ingen spec, ingen kode, ingen SQL. Rammerne er afklaret med bruger og står i US-70-rækken: underfane i dashboardets Administration-fane, gated på `admin`/`manage_tasks`, ingen `completed_at`-kolonne (`end_date` bruges), og "marker som færdig" er UDE af scope. Ingen åbne spørgsmål. Rykket foran US-62/US-63, fordi den hverken kræver SQL eller rører Studerende 2/3's filer - i modsætning til de to, der begge er afhængige af koordinering. **Bemærk:** visningen er tom, indtil nogen bygger US-39's "marker som færdig" (Studerende 3's domæne) - testdata må sættes manuelt i Supabase indtil da.

**3. US-62 – Granulære skriverettigheder i Datalayer.** SQL'en er færdigskrevet og ligger klar i `docs/migrations/us-62-datalayer-write-privileges.sql`. Ingen åbne spørgsmål. Mangler kun frontend-gatingen i Studerende 2's `DataLayerPage.tsx` + 42501-beskeder i `categoryApi.ts` — se filens FRONTEND-afsnit. Privilegiet `manage_datalayer` findes allerede i dropdownen, så det kan tildeles FØR SQL'en køres.

**4. US-63 – Granulære skriverettigheder i Opgaver.** Stadig **BLOKERET**. SQL'en er skrevet, men ligger med "MÅ IKKE KØRES ENDNU"-header i `docs/migrations/us-63-tasks-write-privileges.sql`, hvor de tre ubesvarede spørgsmål står i toppen: (1) skal `task_assignees` forblive selvbetjening, (2) må en tilmeldt menig bruger ændre opgavestatus, (3) hvad gør vi med Studerende 3's ubrugte `task_rooms.required_role_id`. Studerende 3 arbejder desuden aktivt i domænet (commit 2026-09-11).
...
| US-62 | Granulære skriverettigheder i Datalayer | Medium | Mangler (SQL klar) | Tilhører nu Studerende 1 (ikke Studerende 2). **Kørselsklar SQL: `docs/migrations/us-62-datalayer-write-privileges.sql`** - gater `locations`/`data_layer_categories`/`data_layer_items` bag nyt `manage_datalayer`, læsning forbliver åben. Ingen åbne spørgsmål. Privilegiet er allerede tilføjet i `privilegeApi.ts` (2026-09-11), så roller kan få det tildelt inden SQL'en køres - ellers mister alle menige medlemmer skriveadgang på én gang. Filen indeholder en "hvem mister adgang"-query til at tjekke det først. Mangler stadig frontend-gating i Studerende 2's `DataLayerPage.tsx`/komponenter + 42501-beskeder i `categoryApi.ts` |
| US-63 | Granulære skriverettigheder i Opgaver | Medium | Blokeret (SQL parkeret) | Tilhører nu Studerende 1 (ikke Studerende 3). **SQL: `docs/migrations/us-63-tasks-write-privileges.sql` - MÅ IKKE KØRES ENDNU.** Tre ubesvarede spørgsmål i filens header, og Studerende 3 arbejder aktivt i domænet. Revideret 2026-09-11 mod den faktiske database: `deleteTask` findes nu (modsat noten fra 2026-09-10), og `task_rooms` er tilkommet med **fire** policies - de to engelske duplikater (`Users can create/view task rooms...`) gør en gating af `for all`-policyen virkningsløs, fordi policies OR'es, og skal droppes samtidig |
...
9. **US-70** — Se afsluttede opgaver (ad-hoc tilføjet 2026-09-14). Lagt FØR US-62/US-63, fordi den er ren frontend: ingen SQL, ingen ændringer i Studerende 2/3's filer, ingen åbne spørgsmål. Bruger det eksisterende `manage_tasks`-privilegie og læser via de RLS-policies, der allerede findes.
10. **US-62 + US-63** — Granulære skriverettigheder i Datalayer/Opgaver (Fase 2, samme mønster som US-11-13 - tilhører Studerende 1, ikke Studerende 2/3). Bevidst rykket til her, EFTER US-59/60/61: undgår at RLS-policies på Datalayer/Opgave-tabellerne skal rettes til igen når US-59 ændrer medlemskabsmodellen, og reducerer risikoen for at kollidere med Studerende 2/3's igangværende arbejde i de tabeller/komponenter. Kørselsklar SQL ligger nu i `docs/migrations/` — US-62 er klar, US-63 er blokeret. Se afsnittet "Fase 2-spec … flyttet til `docs/migrations/`" nedenfor.
11. ~~**US-02 polish** — vis "ingen organisation"-tilstand i UI~~ ✅ (banner med link til `/organisation`, som nu rummer både opret- og anmod-flow)
12. ~~**US-56 + US-57** — Nyheder (lavest prioritet, ingen afhængigheder — gøres sidst)~~ ✅ (gjort ud af rækkefølge, 2026-09-10, da US-62/63 var blokeret af Tasks-domænet). US-57 efterfølgende udgået og rullet tilbage 2026-09-11 — kun US-56 (manuelt oprettede nyheder) står tilbage.
...
## Fase 2-spec (US-62 + US-63) — flyttet til `docs/migrations/`
