...
**3. Fase 3 – Granulære CRUD-privilegier. ← NÆSTE OPGAVE (trin 1-4 kørt 15/09, trin 5-6 venter).** Erstatter hvert domænes ene `manage_X`-privilegie med separate `create_X`/`read_X`/`update_X`/`delete_X` (kun de operationer der reelt findes pr. domæne), så en rolle fx kan have Create uden Delete. Read er nu også et rigtigt, tildelbart privilegie i stedet for åbent for alle - løst med en ny beskyttet standardrolle "Medlem" (ligesom "Admin"), der auto-tildeles ved medlemskab og som andre roller falder tilbage til ved sletning, så ingen bliver rolleløs. Se afsnittet "Fase 3 - granulære CRUD-privilegier" nederst for den fulde spec.
- **SQL trin 1-4 KØRT og bekræftet i Supabase 15/09** (Medlem-standardrolle, Roller/Organisation, Medlemskab/Medlemmer/Invitationer, Nyheder) - `dbSchema.sql` opdateret til at matche (funktioner, triggere og §16-policies). De fire filer er slettet fra `docs/migrations/` igen, jf. konventionen.
- **Trin 5-6 (Datalayer/Opgaver) skrevet, IKKE kørt:** erstatter (de slettede) `us-62-datalayer-write-privileges.sql`/`us-63-tasks-write-privileges.sql`. `fase3-datalayer-privileges.sql` og `fase3-tasks-privileges.sql` **venter** på hhv. Studerende 2's og Studerende 3's godkendelse (se checkpoint-spørgsmålene i afsnittet nederst), da frontend-gating i deres filer skal på plads samtidig, og Tasks-filen desuden løser 2 af de 3 gamle blokerings-spørgsmål (selvbetjent tilmelding, selvbetjent statusskift via ny `set_task_status`-RPC) men efterlader det tredje (`task_rooms.required_role_id`) åbent.
- **Frontend bygget for de fire kørte domæner (15/09):** `privilegeApi.ts` (nye CRUD-konstanter, `PRIVILEGE_DOMAINS`, grupperet "Tilføj privilegie"-dropdown, ny `useHasAnyPrivilege`), `roleApi.ts` (`MEMBER_ROLE_NAME`, `AssignRoleInput.roleId` ikke længere nullable - "Standard medlem (ingen rolle)" fjernet fra `MembersPanel.tsx`s dropdown), `RolesPrivilegesPanel.tsx` ("Medlem" låst som "Admin"), `AdministrationTab.tsx`/`Dashboard.tsx` (fane-synlighed via `useHasAnyPrivilege`), Nyheder (`NewsPage.tsx`/`NewsDetailPage.tsx`/`NewsCard.tsx`: create/update/delete uafhængige knapper, siden gates nu også af `read_news`). `npm run build` grønt.
- **Mangler:** browser-test af hele flowet (opret org, acceptér anmodning/invitation, tildel/frata privilegier, slet en rolle med medlemmer, escalation-guards), og checkpoint-samtalen med Studerende 2/3 før Datalayer/Opgaver-frontend kan bygges.
...
| US-62 | Granulære skriverettigheder i Datalayer | Medium | Opslugt af Fase 3 | 2026-09-15: designes nu direkte som fuldt CRUD (`create/read/update/delete_datalayer`) i stedet for det gamle enkelt-privilegie. SQL klar i `docs/migrations/fase3-datalayer-privileges.sql` (erstatter den slettede `us-62-...sql`), venter på at frontend-gating i Studerende 2's `DataLayerPage.tsx`/`categoryApi.ts` aftales og bygges - se "Fase 3" nederst |
...
5. `fase3-datalayer-privileges.sql` - stadig **IKKE kørt**, ligger i `docs/migrations/`. **Vent** til Studerende 2 har godkendt frontend-ændringerne (se checkpoint nedenfor)
...
**Testet og bekræftet virkende i browseren 2026-09-15** (efter alle tre rettelser): tildel en custom rolle med kun `create_news`+`read_news` til et medlem → medlemmet kan se/oprette nyheder, profilsiden viser korrekt rollenavn. Resten af listen i "Næste op" (rolle-sletning-fallback, escalation-guards, øvrige domæner) mangler stadig en systematisk gennemtest.
