...
**2. US-74 – Se egne opgaver på dashboardet. ← NÆSTE OPGAVE.** Kun storyen er skrevet (14/09). Ren frontend i `OverviewTab.tsx`: ingen SQL, ingen nye endpoints, ingen ændringer i Studerende 2/3's filer - `getMyTaskIds` + `getTasks` findes allerede. Rammerne er afklaret med bruger og står i US-74-rækken: højst 5 opgaver sorteret efter prioritet så slutdato, ikke-afsluttede kun, rækkerne er bevidst ikke klikbare endnu. Nøgletals-kortene (US-46/47) er udgået og skal ikke bygges op igen.

**3. US-71 → US-72 → US-73 – Notifikationer.** Kun stories skrevet (14/09). Kræver SQL (ny `notifications`-tabel + RLS + triggers) i `docs/migrations/`. Byg trigger-siden i to omgange: medlemskab/invitation, rolleændring og nyheder først (eget domæne, krogene findes allerede i `dbSchema.sql` §15), opgave-hændelserne til sidst, når Tasks-domænet ligger stille.
