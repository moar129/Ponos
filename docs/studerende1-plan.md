...
- **Testet og godkendt i browseren:** hele test-listen A-F inkl. escalation-guards (trin 1-5).
- **Opgaver (trin 6) - godkendt, frontend bygget og SQL kørt+bekræftet 17/09** (var UDSAT til 16/09, se logbogsrækken for 17/09). `dbSchema.sql` opdateret, migrationsfilen slettet. **Browser-test udestår** - se afsnittet "Fase 3" nederst for testliste.

**4. US-71 → US-72 → US-73 – Notifikationer. UDGÅET fra min liste 2026-09-16** — bygges af en anden studerende, ikke mig. Kun stories skrevet (14/09). Kræver SQL (ny `notifications`-tabel + RLS + triggers) i `docs/migrations/`. Byg trigger-siden i to omgange: medlemskab/invitation, rolleændring og nyheder først (eget domæne, krogene findes allerede i `dbSchema.sql` §15), opgave-hændelserne til sidst, når Tasks-domænet ligger stille.
