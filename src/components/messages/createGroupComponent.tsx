      ...
      <div className="bg-white border border-border-gray rounded-xl shadow-xl w-full max-w-md max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-border-gray">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-primary">Opret gruppe</h2>
          </div>
          <button
            type="button"
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          ...
          <div>
            <label className="block text-xs text-secondary uppercase tracking-wide mb-1.5">Gruppenavn</label>
            <input
              type="text"
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs text-secondary uppercase tracking-wide">Deltagere</label>
              {selectedIds.size > 0 && (
            ...
            <div className="border border-border-gray rounded-lg max-h-64 overflow-y-auto">
              {isLoadingMembers ? (
                <div className="flex justify-center py-6">
                  <Loader2 className="w-5 h-5 animate-spin text-accent" />
                </div>
              ) : filteredContacts.length === 0 ? (
                <p className="text-sm text-secondary text-center py-6">Ingen kontakter matcher din søgning.</p>
              ) : (
                      ...
                      <li key={contact.id}>
                        <label className="w-full flex items-center gap-3 p-2.5 cursor-pointer hover:bg-bg-gray/50 transition-colors">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-secondary truncate">{contact.roleName ?? 'Ingen rolle'}</p>
                          </div>
                        </label>
                      </li>
                    );
