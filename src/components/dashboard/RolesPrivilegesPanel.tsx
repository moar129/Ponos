...
function RoleCard({ role, privileges }: RoleCardProps) {
    ...
    return (
        <li className="border border-border-gray rounded-md p-4">
            {isEditing ? (
                <form onSubmit={handleRename} className="flex items-center gap-2 mb-2">
                    <input
                        ...
                        autoFocus
                        className="flex-1 rounded-md border border-border-gray bg-white px-3 py-1.5 text-sm text-primary focus:outline-none focus:border-accent"
                    />
                    <button
                        type="submit"
                        disabled={renaming}
            ...
            <form onSubmit={handleCreatePrivileges} className="flex flex-col gap-2">
                <div className="relative inline-block" ref={pickerRef}>
                    <button
                        ...
                        className="flex items-center justify-between gap-2 min-w-[220px] rounded-md border border-border-gray bg-white px-3 py-1.5 text-sm text-left text-secondary focus:outline-none focus:border-accent"
                    >
                        ...
                        >
                            {pickerGroups.map((group, groupIndex) => (
                                <div key={group.heading ?? `ungrouped-${groupIndex}`}>
                                    {group.heading && (
                                        <p className="px-3 pt-2 pb-1 text-xs font-medium uppercase tracking-wide text-secondary">
                                            {group.heading}
                                        </p>
                                    )}
                                    {group.options.map((p) => (
                                        ...
                                        >
                                            <input
                                                ...
                                                className="rounded border-border-gray bg-white text-accent focus:ring-accent"
                                            />
                                            {p.label}
                                        </label>
                                    ))}
                ...
                <div className="flex flex-wrap items-end gap-2">
                    {selectedPrivileges.includes(CUSTOM_PRIVILEGE_OPTION) && (
                        <input
                            ...
                            autoFocus
                            className="flex-1 min-w-[160px] rounded-md border border-border-gray bg-white px-3 py-1.5 text-sm text-primary focus:outline-none focus:border-accent"
                        />
                    )}
                    <button
                        type="submit"
            ...
            {privilegeFormError && <p className="text-red-600 text-xs mt-2">{privilegeFormError}</p>}
        </li>
    )
}
            ...
            <li>
                <form onSubmit={handleRename} className="flex items-center gap-2">
                    <input
                        ...
                        onChange={(e) => setEditName(e.target.value)}
                        autoFocus
                        className="flex-1 rounded-md border border-border-gray bg-white px-2 py-1 text-xs text-primary focus:outline-none focus:border-accent"
                    />
                    <button
                        type="submit"
                        disabled={renaming}
            ...
            {isAdminPrivilege ? (
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="p-1 rounded-md hover:bg-bg-gray/70 transition-colors"
                    >
