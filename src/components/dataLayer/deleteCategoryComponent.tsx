...
function SubCategoryCheckbox({ category, depth, selectedIds, onToggle }: SubCategoryCheckboxProps) {
  ...
  return (
    <div>
      <div
        className="flex items-center gap-2 py-1.5 rounded-md hover:bg-bg-gray/60 px-1.5"
        style={{ paddingLeft: `${depth * 20 + 6}px` }}
      >
        {hasSubCategories ? (
          <button type="button" onClick={() => setIsOpen(!isOpen)} className="p-0.5 text-secondary hover:text-primary shrink-0">
        ...
        <span className="text-sm text-primary truncate">{category.title}</span>
        {category.items.length > 0 && (
          <span className="text-xs text-secondary shrink-0">({category.items.length} items)</span>
        )}
        {isChecked && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-50 text-red-700 shrink-0 ml-auto">
            Slettes
          </span>
        )}
      </div>
...
export function DeleteCategoryComponent({ isOpen, category, onClose, onDeleted }: DeleteCategoryComponentProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div
        className="bg-white border border-border-gray rounded-xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col"
      ...
      >
        <div className="flex items-start gap-3 p-5 border-b border-border-gray">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-semibold text-primary">Slet "{category.title}"</h2>
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-center gap-2 py-1.5 px-1.5 rounded-md bg-bg-gray/40 border border-border-gray mb-1">
            <span className="w-4 shrink-0" />
            <input type="checkbox" checked disabled className="w-4 h-4 rounded border-border-gray bg-white text-accent shrink-0" />
            <span className="text-sm font-medium text-primary truncate">{category.title}</span>
            {category.items.length > 0 && (
              <span className="text-xs text-secondary shrink-0">({category.items.length} items)</span>
            )}
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-50 text-red-700 shrink-0 ml-auto">
          ...
          {category.subCategories.length > 0 ? (
            ...
            <p className="text-xs text-secondary mt-2">Ingen underkategorier.</p>
          )}
        </div>

        <div className="flex items-center justify-between gap-3 p-4 border-t border-border-gray">
